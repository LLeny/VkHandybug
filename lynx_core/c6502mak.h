...
#define xZEROPAGE()               \
    {                             \
        mOperand = CPU_PEEK(mPC); \
        mPC++;                    \
    }
#define xZEROPAGE_X()                  \
    {                                  \
        mOperand = CPU_PEEK(mPC) + mX; \
        mPC++;                         \
        mOperand &= 0xff;              \
    }
#define xZEROPAGE_Y()                  \
    {                                  \
        mOperand = CPU_PEEK(mPC) + mY; \
        mPC++;                         \
        mOperand &= 0xff;              \
    }
#define xABSOLUTE_X()              \
...
#define xINDIRECT_ABSOLUTE_X()          \
    {                                   \
        mOperand = CPU_PEEKW(mPC);      \
        mPC += 2;                       \
        mOperand += mX;                 \
        mOperand &= 0xffff;             \
        mOperand = CPU_PEEKW(mOperand); \
    }
#define xRELATIVE()                           \
    {                                         \
        mOperand = CPU_PEEK(mPC);             \
        mPC++;                                \
        mOperand = (mPC + mOperand) & 0xffff; \
    }
#define xINDIRECT_X()                   \
    {                                   \
        mOperand = CPU_PEEK(mPC);       \
        mPC++;                          \
        mOperand = mOperand + mX;       \
        mOperand &= 0x00ff;             \
        mOperand = CPU_PEEKW(mOperand); \
    }
#define xINDIRECT_Y()                   \
    {                                   \
        ...
        mPC++;                          \
        mOperand = CPU_PEEKW(mOperand); \
        mOperand = mOperand + mY;       \
        mOperand &= 0xffff;             \
    }
#define xINDIRECT_ABSOLUTE()            \
    {                                   \
        mOperand = CPU_PEEKW(mPC);      \
        mPC += 2;                       \
        mOperand = CPU_PEEKW(mOperand); \
...
#define PULL(m)                     \
    {                               \
        mSP++;                      \
        mSP &= 0xff;                \
        m = CPU_PEEK(mSP + 0x0100); \
    }
#define PUSH(m)                    \
    {                              \
        CPU_POKE(0x0100 + mSP, m); \
        mSP--;                     \
        mSP &= 0xff;               \
    }
//
