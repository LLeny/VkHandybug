...
bool CSusie::ContextSave(FILE *fp)
{
    ...
    if (!fprintf(fp, "CSusie::ContextSave"))
        return 0;

    if (!fwrite(&mTMPADR, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mTILTACUM, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mHOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mVOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mVIDBAS, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mCOLLBAS, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mVIDADR, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mCOLLADR, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mSCBNEXT, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mSPRDLINE, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mHPOSSTRT, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mVPOSSTRT, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mSPRHSIZ, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mSPRVSIZ, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mSTRETCH, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mTILT, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mSPRDOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mSPRVPOS, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mCOLLOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mVSIZACUM, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mHSIZACUM, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mHSIZOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mVSIZOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mSCBADR, sizeof(UUWORD), 1, fp))
        return 0;
    if (!fwrite(&mPROCADR, sizeof(UUWORD), 1, fp))
        return 0;

    if (!fwrite(&mMATHABCD, sizeof(TMATHABCD), 1, fp))
        return 0;
    if (!fwrite(&mMATHEFGH, sizeof(TMATHEFGH), 1, fp))
        return 0;
    if (!fwrite(&mMATHJKLM, sizeof(TMATHJKLM), 1, fp))
        return 0;
    if (!fwrite(&mMATHNP, sizeof(TMATHNP), 1, fp))
        return 0;

    if (!fwrite(&mSPRCTL0_Type, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL0_Vflip, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL0_Hflip, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL0_PixelBits, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mSPRCTL1_StartLeft, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL1_StartUp, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL1_SkipSprite, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL1_ReloadPalette, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL1_ReloadDepth, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL1_Sizing, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCTL1_Literal, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mSPRCOLL_Number, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRCOLL_Collide, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mSPRSYS_StopOnCurrent, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_LeftHand, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_VStretch, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_NoCollide, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_Accumulate, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_SignedMath, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_Status, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_UnsafeAccess, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_LastCarry, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_Mathbit, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mSPRSYS_MathInProgress, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mSUZYBUSEN, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mSPRINIT, sizeof(TSPRINIT), 1, fp))
        return 0;

    if (!fwrite(&mSPRGO, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mEVERON, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(mPenIndex, sizeof(UBYTE), 16, fp))
        return 0;

    if (!fwrite(&mLineType, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mLineShiftRegCount, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mLineShiftReg, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mLineRepeatCount, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mLinePixel, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mLinePacketBitsLeft, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mCollision, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mLineBaseAddress, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mLineCollisionAddress, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mJOYSTICK, sizeof(TJOYSTICK), 1, fp))
        return 0;
    if (!fwrite(&mSWITCHES, sizeof(TSWITCHES), 1, fp))
        return 0;
...
bool CSusie::ContextLoad(LSS_FILE *fp)
{
    ...
    if (!lss_read(teststr, sizeof(char), 19, fp))
        return 0;
    if (strcmp(teststr, "CSusie::ContextSave") != 0)
        return 0;

    if (!lss_read(&mTMPADR, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mTILTACUM, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mHOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mVOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mVIDBAS, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mCOLLBAS, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mVIDADR, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mCOLLADR, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mSCBNEXT, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mSPRDLINE, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mHPOSSTRT, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mVPOSSTRT, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mSPRHSIZ, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mSPRVSIZ, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mSTRETCH, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mTILT, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mSPRDOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mSPRVPOS, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mCOLLOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mVSIZACUM, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mHSIZACUM, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mHSIZOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mVSIZOFF, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mSCBADR, sizeof(UUWORD), 1, fp))
        return 0;
    if (!lss_read(&mPROCADR, sizeof(UUWORD), 1, fp))
        return 0;

    if (!lss_read(&mMATHABCD, sizeof(TMATHABCD), 1, fp))
        return 0;
    if (!lss_read(&mMATHEFGH, sizeof(TMATHEFGH), 1, fp))
        return 0;
    if (!lss_read(&mMATHJKLM, sizeof(TMATHJKLM), 1, fp))
        return 0;
    if (!lss_read(&mMATHNP, sizeof(TMATHNP), 1, fp))
        return 0;

    if (!lss_read(&mSPRCTL0_Type, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL0_Vflip, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL0_Hflip, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL0_PixelBits, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mSPRCTL1_StartLeft, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL1_StartUp, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL1_SkipSprite, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL1_ReloadPalette, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL1_ReloadDepth, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL1_Sizing, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCTL1_Literal, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mSPRCOLL_Number, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRCOLL_Collide, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mSPRSYS_StopOnCurrent, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_LeftHand, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_VStretch, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_NoCollide, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_Accumulate, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_SignedMath, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_Status, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_UnsafeAccess, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_LastCarry, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_Mathbit, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mSPRSYS_MathInProgress, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mSUZYBUSEN, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mSPRINIT, sizeof(TSPRINIT), 1, fp))
        return 0;

    if (!lss_read(&mSPRGO, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mEVERON, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(mPenIndex, sizeof(UBYTE), 16, fp))
        return 0;

    if (!lss_read(&mLineType, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mLineShiftRegCount, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mLineShiftReg, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mLineRepeatCount, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mLinePixel, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mLinePacketBitsLeft, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mCollision, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mLineBaseAddress, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mLineCollisionAddress, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mJOYSTICK, sizeof(TJOYSTICK), 1, fp))
        return 0;
    if (!lss_read(&mSWITCHES, sizeof(TSWITCHES), 1, fp))
        return 0;
        ...
        if (!mSPRCTL1_SkipSprite)
        {
            ...
            for (int loop = 0; loop < 4; loop++)
            {
                ...
                if (superclip)
                {
                    ...
                    {
                    case 3:
                        if ((sprite_h >= screen_h_start || sprite_h < world_h_mid) && (sprite_v < screen_v_end || sprite_v > world_v_mid))
                            render = TRUE;
                        break;
                    case 2:
                        if ((sprite_h >= screen_h_start || sprite_h < world_h_mid) && (sprite_v >= screen_v_start || sprite_v < world_v_mid))
                            render = TRUE;
                        break;
                    case 1:
                        if ((sprite_h < screen_h_end || sprite_h > world_h_mid) && (sprite_v >= screen_v_start || sprite_v < world_v_mid))
                            render = TRUE;
                        break;
                    default:
                        if ((sprite_h < screen_h_end || sprite_h > world_h_mid) && (sprite_v < screen_v_end || sprite_v > world_v_mid))
                            render = TRUE;
                        break;
                    }
                }
                ...
                if (render)
                {
                    ...
                    for (;;)
                    {
                        ...
                        if (mSPRSYS_VStretch)
                            mSPRVSIZ.Word += mSTRETCH.Word * pixel_height;
...
inline ULONG CSusie::LineGetBits(ULONG bits)
{
    ...
    if (mLinePacketBitsLeft <= bits)
        return 0; // <= fixes issues with polygones "demo006"
