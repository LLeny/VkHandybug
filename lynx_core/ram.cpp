    ...
    : mSystem{parent}
{
    ...
    if (filesize)
    {
        ...
        if (header.magic[0] != 'B' || header.magic[1] != 'S' || header.magic[2] != '9' || header.magic[3] != '3')
        {
            ...
            lynxerr.Description() << "The image you selected was not a recognised homebrew format." << std::endl
...
{
    if (!fprintf(fp, "CRam::ContextSave"))
        return 0;
    if (!fwrite(mRamData, sizeof(UBYTE), RAM_SIZE, fp))
        return 0;
    return 1;
}
...
bool CRam::ContextLoad(LSS_FILE *fp)
{
    ...
    if (!lss_read(teststr, sizeof(char), 17, fp))
        return 0;
    if (strcmp(teststr, "CRam::ContextSave") != 0)
        return 0;

    if (!lss_read(mRamData, sizeof(UBYTE), RAM_SIZE, fp))
        return 0;
    mFileSize = 0;
    return 1;
}
