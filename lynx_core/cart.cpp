    ...
    : mSystem{parent}
{
    ...
    if (bank0size == 1)
        bank0size = 0; // workaround ...
    if (bank1size == 1)
        bank1size = 0; // workaround ...
    ...
    if (bank0size == 0)
        bank0size = 1; // workaround ...
    if (bank1size == 0)
        bank1size = 1; // workaround ...
...
bool CCart::ContextSave(FILE *fp)
{
    ...
    if (!fprintf(fp, "CCart::ContextSave"))
        return 0;
    if (!fwrite(&mCounter, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mShifter, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mAddrData, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mStrobe, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mShiftCount0, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mCountMask0, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mShiftCount1, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mCountMask1, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mBank, sizeof(EMMODE), 1, fp))
        return 0;
    if (!fwrite(&mWriteEnableBank0, sizeof(ULONG), 1, fp))
        return 0;
    if (!fwrite(&mWriteEnableBank1, sizeof(ULONG), 1, fp))
        return 0;

    if (!fwrite(&mCartRAM, sizeof(ULONG), 1, fp))
        return 0;
    if (mCartRAM)
    {
        if (!fwrite(&mMaskBank1, sizeof(ULONG), 1, fp))
            return 0;
        if (!fwrite(mCartBank1, sizeof(UBYTE), mMaskBank1 + 1, fp))
            return 0;
    }
    return 1;
}
...
bool CCart::ContextLoad(LSS_FILE *fp)
{
    ...
    if (!lss_read(teststr, sizeof(char), 18, fp))
        return 0;
    if (strcmp(teststr, "CCart::ContextSave") != 0)
        return 0;
    if (!lss_read(&mCounter, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mShifter, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mAddrData, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mStrobe, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mShiftCount0, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mCountMask0, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mShiftCount1, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mCountMask1, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mBank, sizeof(EMMODE), 1, fp))
        return 0;
    if (!lss_read(&mWriteEnableBank0, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mWriteEnableBank1, sizeof(ULONG), 1, fp))
        return 0;

    if (!lss_read(&mCartRAM, sizeof(ULONG), 1, fp))
        return 0;
    if (mCartRAM)
    {
        if (!lss_read(&mMaskBank1, sizeof(ULONG), 1, fp))
            return 0;
        delete[] mCartBank1;
        mCartBank1 = new UBYTE[mMaskBank1 + 1];
        if (!lss_read(mCartBank1, sizeof(UBYTE), mMaskBank1 + 1, fp))
            return 0;
    }
    return 1;
}
...
bool CCart::ContextLoadLegacy(LSS_FILE *fp)
{
    ...
    if (strcmp(teststr, "CCart::ContextSave") != 0)
        return 0;
    if (!lss_read(&mRotation, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mHeaderLess, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mCounter, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mShifter, sizeof(ULONG), 1, fp))
    ...
    if (!lss_read(&mMaskBank0, sizeof(ULONG), 1, fp))
        return 0;
    if (!lss_read(&mMaskBank1, sizeof(ULONG), 1, fp))
        return 0;
    ...
    mCartBank1 = new UBYTE[mMaskBank1 + 1];
    if (!lss_read(mCartBank0, sizeof(UBYTE), mMaskBank0 + 1, fp))
        return 0;
    if (!lss_read(mCartBank1, sizeof(UBYTE), mMaskBank1 + 1, fp))
        return 0;
    return 1;
