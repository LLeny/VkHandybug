...
void CEEPROM::UpdateEeprom(UWORD cnt)
{
    ...
    { // Rising edge
        ...
        case EE_START:
            ...
            case 0x2:
                if (type & 0x80)
                    readdata = ((unsigned char *)romdata)[addr];
                else
...
bool CEEPROM::ContextSave(FILE *fp)
{
    if (!fprintf(fp, "CEEPROM::ContextSave"))
        return 0;
    if (!fwrite(romdata, sizeof(UBYTE), 1024, fp))
        return 0;
    return 1;
}
...
bool CEEPROM::ContextLoad(LSS_FILE *fp)
{
    ...
    if (!lss_read(teststr, sizeof(char), 20, fp))
        return 0;
    if (strcmp(teststr, "CEEPROM::ContextSave") != 0)
        return 0;

    if (!lss_read(romdata, sizeof(UBYTE), 1024, fp))
        return 0;
