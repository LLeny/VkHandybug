...
struct MemoryEditor
{
    ...
    ImU8 (*ReadFn)(const ImU8 *data, size_t off, void *context);    // = 0      // optional handler to read bytes.
    void (*WriteFn)(ImU8 *data, size_t off, ImU8 d, void *context); // = 0      // optional handler to write bytes.
    bool (*HighlightFn)(const ImU8 *data, size_t off);              //= 0      // optional handler to return Highlight property (to support non-contiguous highlighting).
    void *HandlersContext;
            ...
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
            {
                ...
                for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
                {
                    ...
                    if (DataEditingAddr == addr)
                    {
                        ...
                        if (DataEditingTakeFocus)
                        {
                            ...
                            sprintf(DataInputBuf, format_byte, ReadFn ? ReadFn(mem_data, addr, HandlersContext) : mem_data[addr]);
                        }
                        struct UserData
                        {
                        ...
                        sprintf(user_data.CurrentBufOverwrite, format_byte, ReadFn ? ReadFn(mem_data, addr, HandlersContext) : mem_data[addr]);
                        ...
                        {
                            if (WriteFn)
                                WriteFn(mem_data, addr, (ImU8)data_input_value, HandlersContext);
                            else
                ...
                if (OptShowAscii)
                {
                    ...
                    for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
                    {
                        ...
                        unsigned char c = ReadFn ? ReadFn(mem_data, addr, HandlersContext) : mem_data[addr];
    ...
    void DrawPreviewData(size_t addr, const ImU8 *mem_data, size_t mem_size, ImGuiDataType data_type, DataFormat data_format, char *out_buf, size_t out_buf_size) const
    {
        ...
        if (ReadFn)
            for (int i = 0, n = (int)size; i < n; ++i)
                buf[i] = ReadFn(mem_data, addr + i, HandlersContext);
        else
