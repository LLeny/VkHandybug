...
VkImageMemoryBarrier image_memory_barrier(VkImageLayout oldLayout, VkImageLayout newLayout, VkImage image);
VkComputePipelineCreateInfo computepipeline_create_info(VkPipelineLayout layout, VkPipelineCreateFlags flags = 0);
VkDescriptorSetAllocateInfo descriptorset_allocate_info(VkDescriptorPool descriptorPool, const VkDescriptorSetLayout *pSetLayouts, uint32_t descriptorSetCount);
...
VkDescriptorPoolCreateInfo descriptor_pool_create_info(VkDescriptorPoolCreateFlags flags, uint32_t maxSets, uint32_t poolSizeCount, VkDescriptorPoolSize *poolsize);
